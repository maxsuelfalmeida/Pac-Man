...
/**
 * This class is responsable for processing moviment and collisions, for couting points, for
...
public class Engine {
    ...
    public void elementsUpdate(Board board, PacMan pacMan, ArrayList<Ghost> ghosts, ArrayList<Element> foods) {
        ...
        for(Ghost g : ghosts) {
            ...
            if(currMiliTime - g.vulnerableTime > 8000) {
                g.mode = Mode.NORMAL;
                eatenGhosts = 0;
                g.setGhostSpeed(speedFactor);
            }
            
            /* When one ghost get back to the ghosts' cage, it turn back in the 
               normal mode*/
    ...
    public void shiftDirection(Element elem, Board board) {
        ...
        {
            case LEFT:
                elem.direction = (!board.cells.get(28 * row + col - 1).isWall(board.maze) 
                             && y >= (28 * row - 7 + LengthConstants.BACKGROUND_DOWNWARD_OFFSET) 
                             && (y + 28) <= (28 * (row + 1) + 7 + LengthConstants.BACKGROUND_DOWNWARD_OFFSET)) 
            ...
            case UP:
                elem.direction = (!board.cells.get(28 * row + col - 28).isWall(board.maze)
                              && x >= (28 * col - 7) && (x + 28) <= (28 * (col + 1) + 7)) ? elem.nextDirection : elem.direction;
                break;
